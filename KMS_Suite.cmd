...
@echo off
set ver=v7.5
title KMS/2038 ^& Digital ^& Online Activation Suite %ver% by Reave
...
echo.                                                             
echo   [4;44mKMS/2038 ^& Digital ^& Online Activation Suite %ver% - Reave
echo. [44m[m                                                    
echo   [33mSUPPORT MICROSOFT PRUDUCTS:[0m
...
echo.
Echo.  [94m[2] ACTIVATION START FOR WINDOWS 10 (Digital ^& KMS38 Activation Method)[0m
echo.
Echo.  [97m[3] ACTIVATION START FOR WINDOWS 10 ^& OFFICE (Online Activation Method)[0m
Echo.
Echo.  [95m[4] WINDOWS ^& OFFICE ACTIVATION STATUS CHECK[0m
echo.
Echo.  [33m[5] OFFICE 2016 / 2019 PRODUCTS CONVERT RETAIL TO VOLUME[0m
echo.
Echo.  [32m[6] KMS ^& DIGITAL ^& ONLINE ACTIVATION VISIT HELP WEBSITE [0m
Echo.
Echo.  [96m[7] EXIT[0m
Echo.
echo [37m==========================================================================================[1m
...
echo                                                                        %dd%.%mm%.%yy% ^- %mytime%
echo.                                                             
echo  KMS/2038 ^& Digital ^& Online Activation Suite %ver% - Reave
echo.                                                    
...
echo          PC NAME : %computername%
echo===========================================================================================
echo.  [1] ACTIVATION START FOR WINDOWS 10 ^& OFFICE (KMS Inject Method)
echo.
Echo.  [2] ACTIVATION START FOR WINDOWS 10 (Digital ^& KMS38 Activation Method)
echo.
Echo.  [3] ACTIVATION START FOR WINDOWS 10 ^& OFFICE (Online Activation Method)
Echo.
Echo.  [4] WINDOWS ^& OFFICE ACTIVATION STATUS CHECK
echo.
Echo.  [5] OFFICE 2016 / 2019 PRODUCTS CONVERT RETAIL TO VOLUME
echo.
Echo.  [6] KMS ^& DIGITAL ^& ONLINE ACTIVATION VISIT HELP WEBSITE
Echo.
Echo.  [7] EXIT
Echo.
echo===========================================================================================
...
:Online
cls
title KMS (Online) Aktivasyon Windows ^& Office %ver% by Reave
...
echo                                                                        %dd%.%mm%.%yy% ^- %mytime%
echo.
echo  KMS (Online) Activation Windows ^& Office %ver% - Reave
echo.                                                       
echo  SUPPORT MICROSOFT PRUDUCTS:
echo  Windows 7 (VL) - Windows 8 / 8.1 / 10
echo  Windows Server 2008 / 2012 / 2012 R2 / 2016 / 2019
...
echo No KMS server available, & pause & goto:EOF)
set KMS_IP=!server[%server_num%]!
echo Activation process started via %KMS_IP% &echo.
if defined PlusOnlineKMS (
echo. &echo Checking %KMS_IP%... &echo.
ping %KMS_IP% -n 1 -w 20000 > nul || (
echo Ping unsuccessfull for %KMS_IP%  Trying next server.
set /a server_num+=1
goto :server
)
)
...
if %loc_off15% equ 0 if %loc_off16% equ 0 if %loc_off19% equ 0 (set "0ff1ce15="&set "aword=") else (set "aword=Unsupported KMS Client")
if not defined 0ff1ce15 if %winbuild% GEQ 9200 (echo.)
wmic path %spp% where (Description like '%%KMSCLIENT%%') get Name %_Nul_2% | findstr /i Windows %_Nul_1% && (set WinVL=1) || (echo.&echo Unsupported KMS Client Windows Detected...)
...
if %gpr% gtr 259200 (echo Windows 10 EnterpriseG/EnterpriseGN Activation Successful&echo Remaining Period: %gpr2% days ^(%gpr% minutes^)&exit /b)
if %gpr% equ 259200 (
...
:Exit
echo.
echo MSGBOX "SPECIAL THANKS : TNCTR Family - CODYQX4, abbodi1406, qewlpal, s1ave77", vbInformation,"..:: Reave ::.."  > %temp%\TEMPmessage.vbs
